...
func commitEntryToFeedItem(ctx *context.Context, act *activities_model.Action, commitIndex int, commit *repository.PushCommit) (item *feeds.Item, err error) {
	...
	return &feeds.Item{
		...
		Id:      fmt.Sprintf("%d-%d: %s", act.ID, commitIndex, commitURL),
...
// commitsToFeedItems convert commits from gitea's Action to feeds Item
func commitsToFeedItems(ctx *context.Context, actions activities_model.ActionList) (items []*feeds.Item, err error) {
	for _, act := range actions {
		if act.OpType != activities_model.ActionCommitRepo && act.OpType != activities_model.ActionMirrorSyncPush {
			continue
		}
		act.LoadActUser(ctx)
		push := templates.ActionContent2Commits(act)
		...
		for index, commit := range push.Commits {
			...
			desc += fmt.Sprintf("<a href=\"%s\">%s</a><br>%s",
