...
func GetUnmergedPullRequestsByHeadInfoMax(ctx context.Context, repoID, max int64, branch string) ([]*PullRequest, error) {
	prs := make([]*PullRequest, 0, 2)
	sess := db.GetEngine(ctx).
		Join("INNER", "issue", "issue.id = pull_request.issue_id").
		Where("head_repo_id = ? AND head_branch = ? AND has_merged = ? AND issue.is_closed = ? AND flow = ? AND issue.`index` <= ?", repoID, branch, false, false, PullRequestFlowGithub, max)
