...
Gitea supports using client certificates for additional security. When setting the `CA_FILE` option in the server section, the HTTPS server started by Gitea will enforce clients supplying a certificate and validating it against one of the PEM encoded CA certificates in `CA_FILE`. TLS connection is not established should the client certificate be expired or otherwise invalid or if not signed by a trusted CA. Currently this feature only affects the TLS channel between client and server, the certificate's details do not affect authentication within Gitea.
