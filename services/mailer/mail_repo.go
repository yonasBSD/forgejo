...
func sendRepoTransferNotifyMailPerLang(lang string, newOwner, doer *user_model.User, emails []string, repo *repo_model.Repository) error {
	...
	destination := locale.TrString("email.repo.transfer.to_you")
	subject := locale.TrString("email.repo.transfer.subject_to_you", doer.DisplayName(), repo.FullName())
	if newOwner.IsOrganization() {
		destination = newOwner.DisplayName()
		subject = locale.TrString("email.repo.transfer.subject_to", doer.DisplayName(), repo.FullName(), destination)
	}
