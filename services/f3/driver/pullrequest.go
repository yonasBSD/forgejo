...
func (o *pullRequest) Put(ctx context.Context) generic.NodeID {
	...
	getRepoID := func(repo *repo_model.Repository) int64 {
		r, err := repo_model.GetRepositoryByOwnerAndName(ctx, repo.OwnerName, repo.Name)
		if err != nil {
			panic(err)
		}
		return r.ID
	...
	pr.HeadRepoID = getRepoID(pr.HeadRepo)
	pr.BaseRepoID = getRepoID(pr.BaseRepo)
