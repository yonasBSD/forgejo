...
func addIssueComment(t *testing.T, repo *repo_model.Repository, issueIndex int, token, text string) {
	urlStr := fmt.Sprintf("/api/v1/repos/%s/%s/issues/%d/comments?token",
		repo.OwnerName, repo.Name, issueIndex)
...
func getNotifications(t *testing.T, token string) []api.NotificationThread {
	req := NewRequest(t, "GET", "/api/v1/notifications").AddTokenAuth(token)
	resp := MakeRequest(t, req, http.StatusOK)
...
func TestAPIRepoCustomWatch(t *testing.T) {
	...
	t.Run("Issues", func(t *testing.T) {
		...
		MakeRequest(t, NewRequest(t, "PUT", "/api/v1/notifications?status-types=unread&status-types=pinned").AddTokenAuth(token), http.StatusResetContent)

		urlStr := fmt.Sprintf("/api/v1/repos/%s/%s/subscription/custom", repo.OwnerName, repo.Name)
		req := NewRequestWithJSON(t, "PUT", urlStr, api.RepoCustomWatchOptions{Issues: true}).AddTokenAuth(token)
	...
	t.Run("PullRequests", func(t *testing.T) {
		...
		urlStr := fmt.Sprintf("/api/v1/repos/%s/%s/subscription/custom", repo.OwnerName, repo.Name)
		req := NewRequestWithJSON(t, "PUT", urlStr, api.RepoCustomWatchOptions{PullRequests: true}).AddTokenAuth(token)
		MakeRequest(t, req, http.StatusOK)
	...
	t.Run("IssuesAndPullRequests", func(t *testing.T) {
		...
		urlStr := fmt.Sprintf("/api/v1/repos/%s/%s/subscription/custom", repo.OwnerName, repo.Name)
		req := NewRequestWithJSON(t, "PUT", urlStr, api.RepoCustomWatchOptions{Issues: true, PullRequests: true}).AddTokenAuth(token)
		MakeRequest(t, req, http.StatusOK)
	...
	t.Run("Releases", func(t *testing.T) {
		urlStr := fmt.Sprintf("/api/v1/repos/%s/%s/subscription/custom", repo.OwnerName, repo.Name)
		req := NewRequestWithJSON(t, "PUT", urlStr, api.RepoCustomWatchOptions{Releases: true}).AddTokenAuth(token)
		MakeRequest(t, req, http.StatusOK)
