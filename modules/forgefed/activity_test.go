...
func Test_NewForgeUserActivity(t *testing.T) {
	content := "User <whoever> did a <thing>"
	sut, err := NewForgeUserActivity(content)
	want := []byte(`{"type":"Note","content":"User <whoever> did a <thing>"}`)
